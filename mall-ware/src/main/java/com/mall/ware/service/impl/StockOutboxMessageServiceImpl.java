...
public class StockOutboxMessageServiceImpl extends ServiceImpl<StockOutboxMessageDao, StockOutboxMessageEntity>
    ...
    private static final Logger log = LoggerFactory.getLogger(StockOutboxMessageServiceImpl.class);
     ...
     * 用户重试就是重复下单。mall-order 同款代码十万单实测：166 个 500 里 86 单其实已落库。
     ...
     *
     * 范围 UPDATE（WHERE status = ? AND ...）走 idx_stock_outbox_ready(status, ...)，
     * 加锁顺序是「二级索引 → 主键」，而且会锁住该 status 下【所有】索引记录，
     * 包括刚认领、根本没超时的行。与此同时 publish() 认领和 markSent() 确认都是
     * 按主键更新、并且改了 status，加锁顺序是「主键 → 二级索引」—— 反序，必然死锁。
     * 与 mall-order 的 outbox 同一份代码、同一个问题（那边十万单实测 7 小时约 900 次死锁；
     * 真实 MySQL 复现：旧写法 30s 41~72 次，本写法 0 次）。这边只是库存 outbox 量小还没撞上。
     *
    ...
    private void markRetryExhaustedDead() {
        for (Long id : idsWhere(new QueryWrapper<StockOutboxMessageEntity>()
                .in("status", OutboxMessageStatus.PENDING, OutboxMessageStatus.FAILED)
    ...
    private List<Long> idsWhere(QueryWrapper<StockOutboxMessageEntity> condition) {
        return this.list(condition.select("id").orderByAsc("id").last("LIMIT " + properties.batchSize()))
                .stream()
                .map(StockOutboxMessageEntity::getId)
                .toList();
    }
