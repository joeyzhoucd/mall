...
public class DefaultFrontendSecurityService implements FrontendSecurityService {
    ...
    public Mono<FrontendAccessDecision> check(ServerWebExchange exchange) {
        ...
        // 【别在这里挡 /actuator/**】试过，无效。这个类是被 FrontendSecurityFilter
        // （一个 GlobalFilter）调用的，而 GlobalFilter 只对「被路由命中的请求」生效；
        // actuator 自己的 handler mapping 优先级更高，/actuator/** 根本走不到路由。
        // 正确的位置是 WebFilter，见 filter/ManagementEndpointGuardFilter。
