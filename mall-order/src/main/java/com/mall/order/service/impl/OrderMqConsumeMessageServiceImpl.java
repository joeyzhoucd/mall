...
public class OrderMqConsumeMessageServiceImpl
        extends ServiceImpl<OrderMqConsumeMessageDao, OrderMqConsumeMessageEntity>
    ...
    private boolean claim(String consumerGroup, String messageKey, String businessType) {
        Date now = new Date();
        OrderMqConsumeMessageEntity entity = new OrderMqConsumeMessageEntity();
        ...
        try {
            ...
            OrderMqConsumeMessageEntity existing = this.getOne(new QueryWrapper<OrderMqConsumeMessageEntity>()
                    .eq("consumer_group", consumerGroup)
                    .eq("message_key", messageKey));
            if (existing == null || existing.getStatus() != null && existing.getStatus() == MqConsumeStatus.SUCCESS) {
                return false;
            }
            return this.update(new UpdateWrapper<OrderMqConsumeMessageEntity>()
    ...
    private void markSuccess(String consumerGroup, String messageKey) {
        Date now = new Date();
        this.update(new UpdateWrapper<OrderMqConsumeMessageEntity>()
