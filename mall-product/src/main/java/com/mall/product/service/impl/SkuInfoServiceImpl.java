...
public class SkuInfoServiceImpl extends ServiceImpl<SkuInfoDao, SkuInfoEntity> implements SkuInfoService {
     ...
     *
     * <h3>这些任务会阻塞，而阻塞 common pool 是 JVM 级的影响</h3>
     * 四个任务跑的是<b>带互斥重建的缓存读</b>，缓存击穿时
     ...
     * 本来就是反模式。<b>这一条是改动的主要理由</b>。
     *
     * <h3>【实测更正】它没有带来可测量的延迟收益，不要拿它当性能优化</h3>
     * 2026-09-08 在集群上做了同时刻 A/B（旧镜像 pod 和新镜像 pod 并存，
     * 同一批真实冷 skuId，两边都先用真实 id 预热过 JIT）：
     * <pre>
     * 10 并发打同一冷 key，三次中位数：
     *   串行 1.197 / 1.706 / 0.996 s      并行 1.306 / 1.216 / 1.201 s
     * 单请求冷 key，三次：
     *   串行 0.076 / 0.033 / 0.047 s      并行 0.077 / 0.040 / 0.039 s
     * </pre>
     * 两组都在噪声里。原先这里写的「最坏多等 1.2 秒」是<b>未经验证的推断</b>，
     * 实测否掉了它 —— 那 1.2 秒在两个版本上都存在，所以它来自
     * 「互斥竞争 + CPU 饱和」，不是来自串行派发。
     * <p>
     * 推断错在两处：<b>四个内层读各自只有几毫秒</b>（Redis 命中或小的索引查询），
     * 300ms 的 lockWait 只在同 key 竞争时才发生；而<b>容器只有 1 个核</b>
     * （CPU limit 500m），4 个短任务并行派发赢不了顺序执行 ——
     * 没有第二个核可用，重叠那几毫秒 IO 只省个位数毫秒。
     * <p>
     * 所以这个改动的价值是<b>正确性和卫生</b>（不再在 JVM 共享的 common pool 上
     * 做阻塞 I/O），以及<b>CPU 配额提高后才会兑现的可伸缩性</b>。
     * 不要在别处引用它来论证性能。
     *
