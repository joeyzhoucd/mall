...
class AttrCategoryFilterTest {
    ...
    void treatsZeroAndBlankAsUnlimited() {
        ...
        assertThat(AttrServiceImpl.positiveLongOrNull(0L))
                .as("0 是「全部分类」的约定值，不能变成 WHERE category_id = 0")
                .isNull();
        assertThat(AttrServiceImpl.positiveLongOrNull("0")).isNull();
        assertThat(AttrServiceImpl.positiveLongOrNull(null)).isNull();
        assertThat(AttrServiceImpl.positiveLongOrNull("")).isNull();
        assertThat(AttrServiceImpl.positiveLongOrNull("   ")).isNull();
        // 负数没有业务含义，同样当不限 —— 而不是拼出一个必然匹配不到的条件
        assertThat(AttrServiceImpl.positiveLongOrNull(-1L)).isNull();
        // 解析不了不抛异常：这只是个筛选条件，前端传脏数据不该变成 500
        assertThat(AttrServiceImpl.positiveLongOrNull("abc")).isNull();
        assertThat(AttrServiceImpl.positiveLongOrNull("3.5")).isNull();
    }
    ...
    void keepsPositiveIds() {
        ...
        assertThat(AttrServiceImpl.positiveLongOrNull("3")).isEqualTo(3L);
        // 服务内部塞进 map 的是 Long
        assertThat(AttrServiceImpl.positiveLongOrNull(35L)).isEqualTo(35L);
        // Integer 也是 Number
        assertThat(AttrServiceImpl.positiveLongOrNull(12)).isEqualTo(12L);
        // 前后有空白（表单直传）
        assertThat(AttrServiceImpl.positiveLongOrNull(" 7 ")).isEqualTo(7L);
        // pms_attr 里 category_id 的真实取值范围是 3..35，两端都要过
        assertThat(AttrServiceImpl.positiveLongOrNull(3L)).isEqualTo(3L);
        assertThat(AttrServiceImpl.positiveLongOrNull(35L)).isEqualTo(35L);
    }
}
